...
test("Duplicate Dropdown", async ({ page }) => {
  ...
  const colorsHasDuplicate =
    new Set(colorsOptions).size !== colorsOptions.length;
  const animalsHasDuplicate =
    new Set(animalsOptions).size !== animalsOptions.length;

  expect(colorsHasDuplicate).toBeTruthy();
  expect(animalsHasDuplicate).toBeFalsy();
});
